...
def optimize_mesh(
    ...
    for it in range(FLAGS.iter + 1):
        ...
        if display_image or save_image:
            ...
            with torch.no_grad():
                ...
                if FLAGS.subdivision > 0:
                    ...
                    img_base = render.render_mesh(glctx, _opt_base, a_mvp, a_campos, a_lightpos, FLAGS.light_power,
                                                  FLAGS.display_res,
                ...
                img_opt = render.render_mesh(glctx, _opt_detail, a_mvp, a_campos, a_lightpos, FLAGS.light_power,
                                             FLAGS.display_res,
                                             num_layers=FLAGS.layers, background=background,
                                             min_roughness=FLAGS.min_roughness)
                img_ref = render.render_mesh(glctx, _opt_ref, a_mvp, a_campos, a_lightpos, FLAGS.light_power,
                                             FLAGS.display_res,
        ...
        for b in range(FLAGS.batch):
            ...
            v_pos = normalized_base_mesh.v_pos.detach().cpu().numpy() if normalized_base_mesh.v_pos is not None else None
            v_nrm = normalized_base_mesh.v_nrm.detach().cpu().numpy() if normalized_base_mesh.v_nrm is not None else None

            t_pos_idx = normalized_base_mesh.t_pos_idx.detach().cpu().numpy() if normalized_base_mesh.t_pos_idx is not None else None
            t_nrm_idx = normalized_base_mesh.t_nrm_idx.detach().cpu().numpy() if normalized_base_mesh.t_nrm_idx is not None else None

            randF = random.randint(0,len(t_pos_idx) - 1)
            v0 = v_pos[t_pos_idx[randF][0]]
            v1 = v_pos[t_pos_idx[randF][1]]
            v2 = v_pos[t_pos_idx[randF][2]]

            n0 = v_nrm[t_nrm_idx[randF][0]]
            n1 = v_nrm[t_nrm_idx[randF][1]]
            n2 = v_nrm[t_nrm_idx[randF][2]]

            avgV = (v0 + v1 + v2) / 3
            avgN = n0 + n1 + n2
            avgN = avgN / np.sqrt(np.sum(avgN**2))

            e01 = v1 - v0
            e01 = e01 / np.sqrt(np.sum(e01 ** 2))

            eye = np.array(avgV + RADIUS * avgN)
            up = np.array(e01)
            at = np.array(avgV)
            a_mv = util.lookAt(eye, at, up)
            mvp[b] = np.matmul(proj_mtx, a_mv).astype(np.float32)
            campos[b] = np.linalg.inv(a_mv)[:3, 3]
            lightpos[b] = eye
        ...
        with torch.no_grad():
            color_ref = render.render_mesh(glctx, _opt_ref, mvp, campos, lightpos, FLAGS.light_power, iter_res,
                                           spp=iter_spp, num_layers=1, background=randomBgColor,
                                           min_roughness=FLAGS.min_roughness)
        ...
        if log_interval and (it % log_interval == 0):
            ...
            print("iter=%5d, img_loss=%.6f, lr=%.5f, time=%.1f ms, rem=%s" %
                  (it, img_loss_avg, optimizer.param_groups[0]['lr'], iter_dur_avg * 1000,
